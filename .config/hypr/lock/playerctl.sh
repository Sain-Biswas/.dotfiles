...
--title)
	...
	if [ -z "$title" ]; then
		...
		truncate_with_ellipsis "$title" 65 # Limit the output to 50 characters
	fi
	;;
--artist)
	...
	if [ -z "$artist" ]; then
		...
		truncate_with_ellipsis "$artist" 65 # Limit the output to 50 characters
	fi
	;;
--status-symbol)
...
--album)
	...
	if [[ -n $album ]]; then
		...
		if [[ -n $status ]]; then
			...
			truncate_with_ellipsis "$album" 65 # Limit the output to 50 characters
		fi
	fi
	;;
--source-symbol)
